...
function ButtonAdToCart ({ productId, count, setCount }) {
  const { setProducts, setCartActive } = useCart()

  const product = dataProducts.filter(product => product.id === productId)[0]
  const priceWithDiscount = (product.UnitPrice - (product.UnitPrice * product.discount / 100)).toFixed(2)

  const updateCart = () => {
    setProducts({
      id: crypto.randomUUID(),
      urlImg: product.carouselThumbnail[0],
      name: product.name,
  ...
  return (
    <button className='btn btn-primary flex items-center justify-center' onClick={() => { updateCart() }}>
      <IconCart />
      <span>Add to cart</span>
    </button>
  )
}
export default ButtonAdToCart
